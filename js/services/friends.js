utils.jq(() => {
  $(function () {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      const loadFriends = () => {
        utils.request(el, api, function(data) {
          for (let item of (data.content || data)) {
            ...
            cell += `<img src="${item.avatar_url || item.avatar || item.icon || default_avatar}" onerror="javascript:this.src='${default_avatar}';"/>`;
      ...
      util.viewportLazyload(el, loadFriends, lazyload);
    }
  });
});
